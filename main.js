...
stopRecordingBtn.addEventListener('click', stopRecording);
    ...
    recorder.onstop = async function (e) {
        // Your existing code to send audio to the server

        // Receive audio response from the server
        const response = await fetch('http://localhost:5555/process_audio');
        const blob = await response.blob();

        // Create a URL for the received blob
        const audioURL = URL.createObjectURL(blob);

        // Set the audio source to the received URL
        recordedAudio.src = audioURL;
